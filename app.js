    ...
    for (var g = 0, len = gameDirs.length; g < len; g++) { // Loop em
        async.waterfall([ // Splash
            ...
            fs.writeFile('data/cache/games.json', JSON.stringify(result), function(error) { // Saves the games object to a file
                ...
                event.sender.send("game_folder_loaded", {
                    game_path: game_folder_path[0]
                }); // Tells application we're done
            });
        });
    }
});
...
ipcMain.on('load_cemu_folder', function(event) {
    ...
    var cemu = {
        folder_path: cemu_folder_path[0],
        exe_path: cemu_folder_path[0] + "\\" + executable,
        params: "-g"
    }; // makes emulator object
    fs.writeFile('data/cache/emulators.json', JSON.stringify({
        "cemu": cemu
    }), function(error) {
